...
LAmodel(LAmodel *min, int index, int par_num) {
   int verbose=0;
   ...
   // Copy SigWc and SigWGP
   SigWc = new covMat*[pu];
   for (int ii=0; ii<pu; ii++) 
   {
	//SigWc[ii]=new covMat(min->d->zt, min->pp->betaU[ii], min->SigWc[ii]->precz, min->SigWc[ii]->precs );
	SigWc[ii]=new covMat(min->SigWc[ii], d->zt, pp->betaU[ii]); 
   }
   SigWGP=new GPmodel*[pu];
   for(int ii=0; ii<pu; ii++)
   {
	SigWGP[ii]=new GPmodel(d->w[ii],SigWc[ii]);
        SigWGP[ii]->logLik = min->SigWGP[ii]->logLik;
   }

  /*
   // Now compute the my_pu over here
   int my_pu;
   if(par_num==1) //betau
   {
	printf("in BetaU\n");
	my_pu=floor(index/(p+q));
	initPrecompute(my_pu);
   }
   else if(par_num==2) // lamUs,lamWs
   {
	printf("in LamUs/Ws\n");
	my_pu = index;
	initPrecompute(my_pu);
   }
   else if(par_num==3) //lamW0s
   {
	printf("in lamW0\n");
	initPrecompute(); 
   } 
...
void initPrecompute(int my_pu) {
	...
	//SigWc=new covMat*[1];
	int ii = my_pu;
  	  SigWc[0]=new covMat(d->zt,pp->betaU[ii],gsl_vector_get(pp->lamUz,ii),
	                      1.0/(1.0/(gsl_vector_get(pp->LamSim,ii)*pp->lamWOs) + 1.0/gsl_vector_get(pp->lamWs,ii)) );
	...
	for(int i=0; i<pu; i++)
	{
		SigWGP[i]=new GPmodel(d->w[i],SigWc[0]);
		SigWGP[i]->logLik = 0.0; 
	}
	printf("Here 2\n");	
	    SigWGP[0]=new GPmodel(d->w[ii],SigWc[0]); 
  ...
  void updateBetaV(int indi, int indj, double val) {
	...
	SigVc->computeGPUDistCov();
  }
  void updateBetaU(int indi, int indj, double val) {
	gsl_vector_set(pp->betaU[indi],indj,val);
	//printf("stored val=%lf as %lf\n",val,gsl_vector_get(pp->betaU[indi],indj));
	if (n>0) { SigUc[indi]->computeDist(); SigUc[indi]->computeCov(); }
	if(0)
	{
	if(GPU==0){SigWc[indi]->computeDist(); SigWc[indi]->computeCov();}
	else {SigWc[indi]->computeGPUDistCov();}
	if (n>0) { SigUWc[indi]->computeDist(); SigUWc[indi]->computeCov(); }
	SigWGP[indi]->calcLogLik();
	}
	else
	{
	SigWc[indi]->computeGPUDistCovLikelihood(SigWGP[indi]->resp,&SigWGP[indi]->logLik);
	//printf("updatebetaU loglik %lf\n",SigWGP[indi]->logLik);
	}
	if (n>0) computeW();
  ...
  void updateLamUz(int ind, double val) {
	...
	SigWc[ind]->precz=gsl_vector_get(pp->lamUz,ind); 
	if(0)
	{
	if(GPU==0){SigWc[ind]->computeDist(); SigWc[ind]->computeCov();}
	else {SigWc[ind]->computeGPUDistCov();}
	if (n>0) {SigUWc[ind]->precz=gsl_vector_get(pp->lamUz,ind); SigUWc[ind]->computeDist(); SigUWc[ind]->computeCov();}
	SigWGP[ind]->calcLogLik();
	}
	else
        {
        SigWc[ind]->computeGPUDistCovLikelihood(SigWGP[ind]->resp,&SigWGP[ind]->logLik);
        }
	if (n>0) computeW();
	if (n>0) computeSigUgW();
  }
